...
<script type="text/javascript">
	...
	function upd_bak() {
		...
		if(TENDER_START_DATE == "") {
			alert("发标日期不能为空！");
			$("#tenderStartDateBtn").focus();
			return;
		}
		if(SUBMIT_DATE == "") {
			alert("报名截止日不能为空！");
		...
		if(AGENT_CO_MANAGER == "") {
			alert("委托公司联系人不能为空！");
			$("#AGENT_CO_MANAGER").focus();
			return;
		}
		if(AGENT_CO_MANAGER_TEL == "") {
			alert("委托公司电话不能为空！");
			$("#AGENT_CO_MANAGER_TEL").focus();
			return;
		}
		if(!isMobile(AGENT_CO_MANAGER_TEL) && !isTelephone(AGENT_CO_MANAGER_TEL)) {
			alert("请输入正确的委托公司电话！");
			$("#AGENT_CO_MANAGER_TEL").focus();
			return;
		}
		if(AGENT_CO_POST_ADDRESS == "") {
			alert("委托公司地址不能为空！");
			$("#AGENT_CO_POST_ADDRESS").focus();
			return;
		}
		if(AGENT_CO_MAIL == "") {
			alert("委托公司邮箱不能为空！");
		...
		if(PROF_CO_MANAGER == "") {
			alert("专业公司联系人不能为空！");
			$("#PROF_CO_MANAGER").focus();
			return;
		}
		if(PROF_CO_MANAGER_TEL == "") {
			alert("专业公司电话不能为空！");
			$("#PROF_CO_MANAGER_TEL").focus();
			return;
		}
		if(!isMobile(PROF_CO_MANAGER_TEL) && !isTelephone(PROF_CO_MANAGER_TEL)) {
			alert("请输入正确的专业公司电话！");
			$("#PROF_CO_MANAGER_TEL").focus();
			return;
		}
		if(PROF_CO_POST_ADDRESS == "") {
			alert("专业公司地址不能为空！");
			$("#PROF_CO_POST_ADDRESS").focus();
			return;
		}
		if(PROF_CO_MAIL == "") {
			alert("专业公司邮箱不能为空！");
		...
		if(!isReal(RECEIPT1_AGENT_COMMISSION)) {
			alert("代理费格式不正确！");
			$("#RECEIPT1_AGENT_COMMISSION").focus();
			return;
		}
		...
		if(RECEIPT1_RECEIVER == "") {
			alert("接受人不能为空！");
			$("#RECEIPT1_RECEIVER").focus();
			return;
		}
		if(RECEIPT1_VALUE_DATE == "") {
			alert("到账日期不能为空！");
			$("#receipt1ValueDateBtn").focus();
			return;
		}
		if(BID_COMMISION == "") {
			alert("评标费不能为空！");
		...
		if(RECEIPT2_DATE == "") {
			alert("签收时间不能为空！");
			$("#receipt2DateBtn").focus();
			return;
		}
		if(RECEIPT2_POST_NO == "") {
			alert("快递编号不能为空！");
			$("#RECEIPT2_POST_NO").focus();
			return;
		}
		if(RECEIPT2_EXPRESS_DATE == "") {
			alert("快递时间不能为空！");
			$("#receipt2ExpressDateBtn").focus();
			return;
		}
	...
	function updBidComp() {
		...
		if(tr != null) {
			...
			var tempBID_BOND = childs[4].value;
			var tempBID_PAYMENT_TYPE = childs[5].value;
			var tempBID_PRICE = childs[6].value;
			var tempBID_CHECK_PRICE = childs[7].value;
			var tempBID_RESULT = childs[8].value;
			var tempBID_WIN_PRICE = childs[9].value;
			var tempBID_APPLY_PRICE = childs[10].value;
			var tempBID_VALUE_DATE = childs[11].value;
			var tempBID_RECEIPT_NO = childs[12].value;
			
			var tempBID_REFUND_SIGN = childs[13].value;
			
			var tempMEMO1 = childs[14].value;
			
			var tempBID_CO_MANAGER = childs[15].value;
			var tempBID_CO_ADD = childs[16].value;
			var tempBID_CO_PS = childs[17].value;
			var tempBID_CO_NO = childs[18].value;
			...
			$("#tempBID_BOND").attr("value", tempBID_BOND);
			$("#tempBID_PAYMENT_TYPE").attr("value", tempBID_PAYMENT_TYPE);
			$("#tempBID_PRICE").attr("value", tempBID_PRICE);
			$("#tempBID_CHECK_PRICE").attr("value", tempBID_CHECK_PRICE);
			$("#tempBID_RESULT").attr("value", tempBID_RESULT);
			$("#tempBID_WIN_PRICE").attr("value", tempBID_WIN_PRICE);
			$("#tempBID_APPLY_PRICE").attr("value", tempBID_APPLY_PRICE);
	...
	function showAddExpertLib() {
		...
		var url = '<c:url value="/bid/showGetAutoBidExpertAction.action"></c:url>' + "?date=" + new Date();
		window.showModalDialog(url, window, "dialogheight:580px;dialogwidth:920px;center:yes;status:0;resizable=no;Minimize=no;Maximize=no");
	}
	...
	function setBidCompList() {
		...
		for(var i = 0; i < rows.length; i++) {
			...
			var BID_BOND = childs[4].value;
			var BID_PAYMENT_TYPE = childs[5].value;
			var BID_PRICE = childs[6].value;
			var BID_CHECK_PRICE = childs[7].value;
			var BID_RESULT = childs[8].value;
			var BID_WIN_PRICE = childs[9].value;
			var BID_APPLY_PRICE = childs[10].value;
			var BID_VALUE_DATE = childs[11].value;
			var BID_RECEIPT_NO = childs[12].value;
			var BID_REFUND_SIGN = childs[13].value;
			var MEMO1 = childs[14].value;
			
			var BID_CO_MANAGER = childs[15].value;
			var BID_CO_ADD = childs[16].value;
			var BID_CO_PS = childs[17].value;
			var BID_CO_NO = childs[18].value;
			...
			td.appendChild(createInput("listBidCompTmp[" + i + "].BID_BOND", BID_BOND));
			td.appendChild(createInput("listBidCompTmp[" + i + "].BID_PAYMENT_TYPE", BID_PAYMENT_TYPE));
			td.appendChild(createInput("listBidCompTmp[" + i + "].BID_PRICE", BID_PRICE));
			td.appendChild(createInput("listBidCompTmp[" + i + "].BID_CHECK_PRICE", BID_CHECK_PRICE));
			td.appendChild(createInput("listBidCompTmp[" + i + "].BID_RESULT", BID_RESULT));
			td.appendChild(createInput("listBidCompTmp[" + i + "].BID_WIN_PRICE", BID_WIN_PRICE));
			td.appendChild(createInput("listBidCompTmp[" + i + "].BID_APPLY_PRICE", BID_APPLY_PRICE));
	...
	<s:hidden name="updateBidDtoOld.BID_EXPERT_LIST" id="BID_EXPERT_LIST_OLD"/>
		...
		<div class="content">
			...
			<s:form id="mainform" name="mainform" method="POST">
				...
				<s:hidden name="updateBidDto.AGENT_CO_MAIL" id="AGENT_CO_MAIL"/>
				...
				<s:hidden name="updateBidDto.PROF_CO_MAIL" id="PROF_CO_MAIL"/>
					...
					<table width="100%" border="0" cellpadding="5" cellspacing="0">
						...
						<tr>
							...
							<td>
								<s:if test='updateBidDto.PROJECT_TYPE == "1"'>
									<input id="radioProjectType" name="projectType" onclick="selectProjectType(this);" checked="checked" type="radio" value="1"/><label>招标</label>
							...
							<td colspan="3">
								<div class="box1_left"></div>
								<div class="box1_center">
									<s:textfield name="updateBidDto.PROJECT_MANAGER" id="PROJECT_MANAGER" cssStyle="width:135px;" maxlength="24" theme="simple"></s:textfield>
								</div>
								<div class="box1_right"></div>
							</td>
						</tr>
						<tr>
							...
							<td colspan="7">
								<div class="box1_left"></div>
								<div class="box1_center">
									<s:textfield name="updateBidDto.PROJECT_NAME" id="PROJECT_NAME" cssStyle="width:880px;" maxlength="300" theme="simple"></s:textfield>
								</div>
								<div class="box1_right"></div>
							</td>
						</tr>
					...
					<table id="tempTable" style="display: none;">
						<tr>
							<td>
								...
								<input type="hidden" id="tempBID_BOND" value="" />
								<input type="hidden" id="tempBID_REFUND_SIGN" value="" />
								<input type="hidden" id="tempBID_PAYMENT_TYPE" value="" />
								<input type="hidden" id="tempBID_PRICE" value="" />
								<input type="hidden" id="tempBID_CHECK_PRICE" value="" />
								<input type="hidden" id="tempBID_RESULT" value="" />
								<input type="hidden" id="tempBID_WIN_PRICE" value="" />
								<input type="hidden" id="tempBID_APPLY_PRICE" value="" />
