	...
	function addExpertComp() {
		...
		for(var i = 0; i < list.length; i++) {
			if(list[i].checked) {
				obj = list[i];
				if(list[i].value != ""){
					if(expertname.indexOf(",'" + list[i].value + "',") == -1){
						expertname += list[i].value + "','";
					}
				}
			}
		}
		...
		expertname = expertname.substring(1, expertname.length - 2);
		//就职公司
		window.dialogArguments.document.getElementById("tmpExpertComp").value = expertname;
		window.close();
	}
</script>
</head>
				...
				<table class="info_tab" width="100%" border="1" cellpadding="5" cellspacing="0">
					...
					<s:iterator id="listExpertComp" value="listExpertComp" status="st1">
						...
						<s:else>
							...
							<td><input name="checkKey" type="checkbox" value="<s:property value="EXPERT_COMP"/>"/></td>
					...
					<td>
						<div class="btn">
							...
							<div class="box1_center">
								<input type="button" class="input80" value="确定" onclick="addExpertComp();"/>
							</div>
